...
const TEAMS_KEY = "cleaningTeams";
const ROTATION_KEY = "cleaningRotation";
...
export async function getCleaningTeams() {
  const teams = await getValue(TEAMS_KEY, []);
  return teams || [];
}

export async function setCleaningTeams(teams) {
  await setValue(TEAMS_KEY, teams);
}

export async function getCleaningRotation() {
  const rotation = await getValue(ROTATION_KEY, null);
  return rotation || null;
}

export async function setCleaningRotation(rotation) {
  await setValue(ROTATION_KEY, rotation);
}
