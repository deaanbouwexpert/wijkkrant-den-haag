...
export default function AdminPage() {
  ...
  // --- Eenmalige import van de schoonmaak-teams en het jaarrooster 2026 ---
  const BULK_TEAMS = [
    {
      name: "Team 1",
      members: [
        "Rob Broekman", "Melissa Broekman", "Susannah Broekman", "Hellen Oudheusden",
        "Kyra Bidjai", "Rocco Navarro", "Boaz Bidjai", "Jozef Broekman",
      ],
    },
    {
      name: "Team 2",
      members: [
        "Cor Brinkers", "Alexandra Brinkers", "Margaretha Pinas", "Kathleen Mohamatsaid",
        "Jan-Willem Versteegh", "Irene Versteegh",
      ],
    },
    {
      name: "Team 3",
      members: ["Timo Swart", "Onne de Jonge", "Maaike de Jonge", "Floor de Jonge", "Christopher", "Geerte den Hollander"],
    },
    {
      name: "Team 4",
      members: [
        "Paul Muntinga", "Rachel Muntinga", "Jennifer Muntinga", "Joshua Muntinga",
        "Marjolein Prosman", "Dennis Prosman", "Jamie & Osmeli Herrera",
      ],
    },
    {
      name: "Team 5",
      members: [
        "Martijn Scherf", "Linda Scherf-Siebert", "Rosa-Lynn Decker", "Rick Feith",
        "Jormain Mossink", "Uacirlene Morais", "Maije Zarse",
      ],
    },
    {
      name: "Team 6",
      members: [
        "Jack van Oudheusden", "Izaac Broekman", "Alice van Oudheusden", "Peter Navarro",
        "Caroline Navarro", "Naomi & Joshua Esser",
      ],
    },
    {
      name: "Team 7",
      members: [
        "Gijs de Jonge", "Diederik Linders", "Ian Prosman", "Athena Wijsman",
        "Graciella Roozenburg", "Nora Broekman", "Deepak Binda",
      ],
    },
    {
      name: "Team 8",
      members: [
        "Charlotte Wiersma (time out)", "Yvonne Peschier", "Ineke Navarro", "Lucas Maring",
        "Camille Maring", "Nathan Navarro", "Maria Dorssen",
      ],
    },
  ];

  const BULK_ROSTER = [
    ["2026-01-12", "Team 1"], ["2026-03-09", "Team 1"], ["2026-05-03", "Team 1"], ["2026-06-29", "Team 1"],
    ["2026-08-24", "Team 1"], ["2026-10-19", "Team 1"], ["2026-12-14", "Team 1"],
    ["2026-01-19", "Team 2"], ["2026-03-16", "Team 2"], ["2026-05-11", "Team 2"], ["2026-07-06", "Team 2"],
    ["2026-08-31", "Team 2"], ["2026-10-26", "Team 2"], ["2026-12-21", "Team 2"],
    ["2026-01-26", "Team 3"], ["2026-03-23", "Team 3"], ["2026-05-18", "Team 3"], ["2026-07-13", "Team 3"],
    ["2026-09-07", "Team 3"], ["2026-11-02", "Team 3"], ["2026-12-28", "Team 3"],
    ["2026-02-02", "Team 4"], ["2026-03-30", "Team 4"], ["2026-05-25", "Team 4"], ["2026-07-20", "Team 4"],
    ["2026-09-14", "Team 4"], ["2026-11-09", "Team 4"], ["2027-01-05", "Team 4"],
    ["2026-02-09", "Team 5"], ["2026-04-06", "Team 5"], ["2026-06-01", "Team 5"], ["2026-07-27", "Team 5"],
    ["2026-09-21", "Team 5"], ["2026-11-16", "Team 5"],
    ["2026-02-16", "Team 6"], ["2026-04-13", "Team 6"], ["2026-06-08", "Team 6"], ["2026-08-03", "Team 6"],
    ["2026-09-28", "Team 6"], ["2026-11-23", "Team 6"],
    ["2026-02-23", "Team 7"], ["2026-04-20", "Team 7"], ["2026-06-15", "Team 7"], ["2026-08-10", "Team 7"],
    ["2026-10-05", "Team 7"], ["2026-11-30", "Team 7"],
    ["2026-03-02", "Team 8"], ["2026-04-27", "Team 8"], ["2026-06-22", "Team 8"], ["2026-08-17", "Team 8"],
    ["2026-10-12", "Team 8"], ["2026-12-07", "Team 8"],
  ];

  const [bulkImportBusy, setBulkImportBusy] = useState(false);

  const bulkImportCleaning = async () => {
    if (!confirm("Alle 8 teams en het volledige jaarrooster 2026 in één keer toevoegen (bestaande teams/data blijven staan, dubbele worden overgeslagen)?")) {
      return;
    }
    ...
    let rosterAdded = 0;
    try {
      const currentTeamsRes = await fetch("/api/teams");
      const currentTeams = currentTeamsRes.ok ? (await currentTeamsRes.json()).teams || [] : [];
      const existingTeamNames = new Set(currentTeams.map((t) => t.name));
      ...
      for (const team of BULK_TEAMS) {
        ...
        const res = await fetch("/api/teams", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-admin-password": pw },
          body: JSON.stringify({ name: team.name, members: team.members }),
        });
        if (res.ok) teamsAdded++;
      }

      const currentRosterRes = await fetch("/api/roster");
      const currentRoster = currentRosterRes.ok ? (await currentRosterRes.json()).roster || [] : [];
      const existingDates = new Set(currentRoster.map((r) => r.date));

      for (const [date, who] of BULK_ROSTER) {
        if (existingDates.has(date)) continue;
        const res = await fetch("/api/roster", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-admin-password": pw },
          body: JSON.stringify({ date, who }),
        });
        if (res.ok) rosterAdded++;
      }

      showToast(`Klaar: ${teamsAdded} team(s) en ${rosterAdded} roosterregel(s) toegevoegd.`);
      loadTeams();
      loadRoster();
    } catch {
      showToast("Er ging iets mis tijdens het importeren — probeer het nog eens.");
    }
      ...
      {!authed ? (
        ...
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <button type="button" className="btn btn-sm btn-outline" onClick={bulkImportCleaning} disabled={bulkImportBusy}>
              <Download size={13} /> {bulkImportBusy ? "Bezig..." : "Teams + jaarrooster 2026 in één keer invullen"}
            </button>
          </div>
